...
def test_каждый_известный_руководитель_находится_поиском(records):
    """Поле ввода обещает поиск по ФИО руководителя — обещание должно
    выполняться на всём наборе, а не на удачном примере.

    До правки поиск смотрел только на название и ИНН, и 150 компаний из 200
    по своему же руководителю не находились.
    """
    from core import repo

    ненайденные = []
    for record in records:
        имя = str(((record.get("foundersInfo") or {}).get("authPerson") or {}).get("name") or "")
        if not имя.strip():
            continue
        инн = str((record.get("baseInfo") or {}).get("inn"))
        # limit побольше: у однофамильцев в выдаче несколько компаний
        найдено = {str((r.get("baseInfo") or {}).get("inn")) for r in repo.search(имя, limit=50)}
        if инн not in найдено:
            ненайденные.append((инн, имя))

    assert not ненайденные, f"по своему руководителю не находятся: {ненайденные[:5]}"


def test_у_предпринимателей_руководителя_не_бывает(records):
    """Пятьдесят ИП из двухсот. Пустое поле у них — устройство формы, а не
    пробел в данных, и подписывать его «Недостаточно данных» нельзя."""
    ип = [
        r
        for r in records
        if str((r.get("baseInfo") or {}).get("shortName") or "").startswith("ИП")
    ]
    assert len(ип) == 50
    for record in ип:
        assert not ((record.get("foundersInfo") or {}).get("authPerson") or {}).get("name")
