...
async def test_текст_приходит_событиями(monkeypatch):
    ...
    assert [с.name for с in события] == ["deal", "token", "token", "done"]


async def test_сверки_чисел_в_потоке_нет(monkeypatch):
    """Убрана намеренно. Замерено на настоящих отчётах: из семи процитированных
    чисел она подтверждала все семь, из пяти посчитанных три помечала как
    выдуманные, а из пяти выдуманных одно пропускала. То есть поощряла
    цитирование и наказывала счёт — ровно то, за чем человек приходит.

    В ядре и в MCP-инструменте `verify_claims` проверка осталась: там её
    просят намеренно и понимают, что «не найдено» ≠ «выдумано».
    """
    события, _ = await прогнать(monkeypatch, ПодставнойАгент([кусок("Исков на 999 999 999 ₽.")]))

    assert "check" not in [с.name for с in события]
...
def test_обычная_ручка_отдаёт_добытое_инструментами(monkeypatch):
    ...
    assert ответ.json() == {
        ...
        "charts": ["balance"],
        "sources": [{"title": "Т", "url": "https://x", "snippet": ""}],
        # То же, что в потоке приходит событием `lookup`: без него клиент
        # не увидит данные, на которые опирается ответ.
        "lookups": [],
