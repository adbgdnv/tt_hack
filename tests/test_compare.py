...
def test_у_каждого_состояния_есть_слова():
    for состояние in (compare.ЧИСТО, compare.УТОЧНИТЬ, compare.ВНИМАНИЕ):
...
def test_у_ип_отсутствие_отчётности_не_пробел():
    ...
    Экран считает пробелы по разделам в состоянии `empty`; раздел, которого
    у ИП не бывает, помечен `not_applicable` и в пробелы не попадает.
    """
    from core.report import State, build
    ...
    финансы = next(р for р in build(ип).sections if р.key == "finances")
    assert финансы.state is State.NOT_APPLICABLE


@нужен_набор
def test_отчёт_не_отдаёт_своей_оценки():
    """Она считается на экране вместе с баннером, одним правилом.

    Пока правил было два — здесь по расхождениям между разделами, на экране
    по сигнальным разделам, — они расходились у 116 компаний из 200, и у
    восемнадцати сервер говорил «вопросов нет» там, где баннер на той же
    странице говорил «обратить внимание».
    """
    from fastapi.testclient import TestClient
    ...
    with TestClient(app) as клиент:
        отчёт = клиент.get("/counterparties/5032257375/report").json()

    assert "verdict" not in отчёт
    assert отчёт["sections"], "вывод строится из разделов — они должны приходить"
