...
def test_общий_ключ_больше_не_принимается(monkeypatch):
    """Сторож на заряженное ружьё.

    В общем `LLM_API_KEY` лежал ключ Groq. Стоило пропасть именному ключу
    OpenRouter — и продукт молча переезжал на gpt-oss: без ошибки, без записи
    в логе, с одним лишь падением качества ответов. Проверено на живом сервере:
    отличить один провайдер от другого снаружи было нечем, счётчик расхода
    OpenRouter обновляется с задержкой и для этого не годится.

    Теперь пропавший ключ — честная ошибка при старте.
    """
    monkeypatch.setenv("LLM_PROVIDER", "groq")
