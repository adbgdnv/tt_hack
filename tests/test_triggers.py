...
def test_условия_сделки_поднимают_своё_даже_на_чужом_вопросе():
    """Намерение живёт один вопрос, условия сделки — весь разговор. Вопрос
    «сколько у них сотрудников» при отсрочке остаётся вопросом человека,
    который даёт отсрочку."""
    from core.deal import Deal, tags
    from core.triggers import order_for

    отсрочка = tags(Deal(scheme="deferral"))
    порядок = [t.key for t in order_for(тройка(), "сколько у них сотрудников", отсрочка)]

    assert порядок[0] == "b"  # финансы — тег отсрочки
