# Это директория проекта по хакатону
tt_hack - общая директория команды 




# Counterparty Checker — контекст проекта

> Этот блок лежит **снаружи** маркеров `Next-Move-Theory-Rules` ниже. Всё, что внутри
> них, затирается при `/nmt-upgrade` — проектные правила держим здесь.

## Что это

Кейс хакатона «Альфа Будущее» (Альфа-Банк): ИИ-агент, который анализирует отчёт о
контрагенте, выявляет риски и отвечает на вопросы **только по входным данным**.
...
| Путь | Что это |
|---|---|
| `data/` | материалы кейсодателя — **не редактировать** |
| `data/case_description.md` | официальное описание кейса |
...
## Данные — что обязательно знать

Проверено на всех 200 компаниях. Эти вещи ломают наивную обработку:

- **JSON и CSV — разные компании.** Пересечение по ОГРН ноль. Всего 200, не 100.
- **`{"$numberLong": "..."}`** вместо числа для сумм больше 2³¹. Затрагивает ровно
  самые крупные дела (4,5 и 3,0 млрд ₽) — `float()` на них падает, и из выборки
  выпадают самые тяжёлые случаи.
- **`аrbitrationDefendant`** среди позитивных факторов написан с кириллической «а»
  (U+0430), среди негативных — с латинской (U+0061). Сравнение строк промахивается.
- **`defandantArbitration`** — опечатка в схеме (должно быть defendant), по всему файлу.
- Два светофора от **разных организаций** (расшифровка — `data/image_2.png`):
  `baseInfo.riskLevel` считает **скоринг банка** по внутренней методологии, которая не
  разглашается (LOW/MEDIUM/HIGH и недокументированный **UNKNOWN** — «нет данных для
  оценки», 6 компаний из 200). `zskRiskLevel` — агрегация платформы «Знай своего клиента»
  **ЦБ РФ** (GREEN 158 / YELLOW 40 / RED 2), и по спеке жёлтый с красным **выводятся в
  интерфейс серым**, то есть неотличимо от «нет данных». Подробности —
  `docs/data_reference.md`.
- Все 200 в статусе `CURRENT`. 50 из 200 — ИП: у них нет учредителей, уставного
  капитала и бухотчётности, отвечать про это надо «у ИП такого не бывает», а не
  «данных нет».
- Негативных риск-кодов всего 15; пять из них срабатывают 1–2 раза на 200 компаний.
- Шум к сигналу в отчёте — **14,4 : 1** (3122 позитивных фактора против 217 негативных
  на всех 200 компаниях; цифры 1529/114 из ранних заметок считались только по JSON).

## Продуктовая рамка

- **Целевая аудитория (со слов кейсодателя): предприниматели и финансисты.** Стоят на
  разных концах покрытия данными: предпринимателю хватает риск-кодов и светофоров (100%),
  финансисту нужна отчётность (64%) и коэффициенты (24%), а у всех 50 ИП отчётности нет
  вовсе. Две разные выдачи: предпринимателю вердикт и калибровка, финансисту цифры и
  ранжирование. Разбор по ситуациям — `docs/roles_situations.md`.
- **Боль:** пользователь приходит за решением «работать или нет», получает сорок полей
  и два светофора, которые считают не тот риск, и должен собрать вывод сам — по одной
  компании за раз.
- **Ход:** второй, независимый вердикт по открытым данным (суды, исполнительные
  производства, реестры ФНС, финансы) с явным указанием, чего каждый светофор не
  покрывает.
- **Опорный пример:** ООО «МАКСМАРКЕТ» — оба светофора зелёные при 2,6 млрд ₽ исков
  в роли ответчика, 54 активных производствах и предстоящем исключении из ЕГРЮЛ.
- Чтение отчёта — издержка, а не задача. Саммари поверх отчёта её удваивает; работает
  вычитание: гасить непроизошедшие события, показывать произошедшие.

## Как вести работу

**Spec Kit прогоняется целиком, без остановок.** Пользователь даёт `/speckit-specify`
с описанием — дальше `/speckit-plan`, `/speckit-tasks`, `/speckit-implement` запускаются
подряд самостоятельно, без вопроса «запускать ли дальше». Спрашивать только там, где
ответ меняет саму работу: развилки в спеке, расхождения с данными, решения о доступах.

Причина: до финала считанные дни, а подтверждение каждого шага конвейера не добавляет
информации — решения принимаются в уточнениях к спеке, а не между командами.

## Ограничения кейса

- LLM — любая, предпочтительно **Qwen** или **GPT-OSS**.
- Критерии приёмки: не выдумывает · использует только входные данные · отвечает на
  большинство вопросов при экспертном тестировании. Эталонного датасета нет.
- Вне скоупа: интеграция с банковскими системами, большая база карточек контрагентов.
- MCP-сервер опционален; веб-интерфейс назван «большим плюсом».
- Внешние открытые источники разрешены (Checko, Rusprofile, ЗаЧестныйБизнес и др.).
- Транзакционных признаков у нас не будет — значит банковский светофор мы не
  объясняем, а дополняем.
- Дизайн-система: `@alfalab/core-components`, см. скилл `alfa-core-components`.

## Что уже неверно

`imported_from_claude.md` строился на тезисе «продукт не даёт вердикт, потому что никто
не хочет за него отвечать». Опровергнуто: вердикт уже есть, причём два, и кейсодатель
разрешил давать свой. Экономика в том файле посчитана для фичи внутри банка и на
внешний продукт не переносится.
