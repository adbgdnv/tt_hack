...
Развёрнут 04.09.2026. Автодеплой его **не трогает** — обновляется вручную.
...
```bash
git -C /opt/tt-hack pull --ff-only && chown -R www-data:www-data /opt/tt-hack
cd /opt/tt-hack && docker compose up -d --build api
curl -s localhost:8000/health | python3 -m json.tool     # ожидается count: 200
```

Через автодеплой это не гоняется: пользователь `ttdeploy` без sudo, а фронт
и бэкенд обновляются с разной частотой.
