...
Автодеплой **выкатывает и бэкенд**: порядок шагов окружение → контейнер `api` → фронт.
Он по-прежнему **не трогает** `/opt/tt-hack-review/`, сервис `tt-hack-vibe-debug`
и контейнер `mcp`.
