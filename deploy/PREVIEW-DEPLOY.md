...
`OPENROUTER_API_KEY` и `GROQ_API_KEY` (repository secrets).
...
**Общего `LLM_API_KEY` больше нет.** У каждого провайдера свой именной ключ.
В общем лежал ключ Groq, и стоило пропасть именному ключу OpenRouter, как
диалог молча переезжал на gpt-oss — без ошибки, без записи в логе, с одним
лишь падением качества ответов. Проверено на живом сервере: снаружи это
не отличить, а счётчик расхода OpenRouter обновляется с задержкой и для
проверки не годится. Теперь пропавший ключ — честная ошибка при старте.
...
### Где собирается и откуда приезжает

**Сборка идёт в раннере GitHub, а не на сервере.** Раньше собиралась здесь,
и это клало сервис: у машины 894 МБ памяти, нагрузка доходила до 27, SSH
переставал отвечать, а отмена выкатки на середине оставила контейнер
«созданным, но не запущенным» — сайт лежал, пока его не подняли руками.

Теперь задание `build` собирает оба образа параллельно и кладёт в `ghcr.io`
под двумя метками: по коммиту и `latest`. Сервер только забирает готовое.

Замерено на первом же прогоне нового пути: нагрузка **0,15–0,44** вместо 27,
SSH отвечает всё время, выкатка укладывается в три минуты вместо пятнадцати.

Сборка в раннере — 20–70 секунд на образ; кэш слоёв хранит сам GitHub.

**Образы публичные.** Это решение, а не недосмотр: закрывать в них нечего —
код открыт, набор данных подключается томом, ключи задаются при запуске.
Взамен на сервере нет ни одной учётной записи к реестру.

**Выкатка ссылается на метку коммита, не на `latest`.** `latest` всегда
указывает на новое, и откатиться по нему невозможно.

### Откат на предыдущую версию

Занимает меньше минуты и ничего не собирает:

```bash
ssh root@72.56.16.44
cd /opt/tt-hack
TAG=<метка-прошлого-коммита> docker compose pull api web
TAG=<метка-прошлого-коммита> docker compose up -d --no-build api web
```

Метку взять из истории прогонов или прямо из реестра:

```bash
gh api /users/adbgdnv/packages/container/tt_hack%2Fapi/versions \
  --jq '.[0:10][] | .metadata.container.tags'
```

`--no-build` обязателен. Без него compose при заминке с образом соберёт его
сам — то есть вернёт ровно ту беду, из-за которой всё переносилось.

Старые версии убираются раз в неделю, последние десять остаются.

### Поток ответа и буферизация

Диалог отвечает потоком: `POST /api/chat/stream` отдаёт server-sent events.
Из-за этого в блоке `location ^~ /api/` **обязателен `proxy_buffering off`** —
иначе nginx копит ответ и отдаёт целиком. Снаружи это выглядит как полностью
исправный сервер, у которого просто нет стриминга, и отлаживается тяжело:
локально всё работает.

Проксирований `/api/` два — на хосте (`deploy/nginx/tt-hack-review.conf`)
и внутри контейнера фронта (`src/web/nginx.conf`, нужен локальному запуску).
Директивы стоят в обоих: править надо оба, иначе поток пропадёт в том окружении,
про которое забыли.

`proxy_read_timeout` там же поднят до 120 с — больше, чем таймаут клиента модели
(90 с). Обрывать раньше, чем это сделает сам сервис, значит показать пользователю
сбой вместо ответа.

Сервис дополнительно шлёт заголовок `X-Accel-Buffering: no` — подстраховка
на случай, если конфиг где-то забыли.

### Поиск во внешних источниках

Включается наличием секрета `TAVILY_API_KEY`. Без него инструмент поиска модели
**не предлагается вовсе** — не заглушка, отвечающая «недоступно»: её описание
стоило бы токенов на каждом вызове при лимите 8 000 в минуту.

Ходим к поставщику по MCP (`mcp.tavily.com`), то есть продукт выступает
MCP-клиентом. Из пяти инструментов их сервера модели описан только поиск: все
пять стоят 2383 токена на каждом вызове.
