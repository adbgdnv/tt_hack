...
## Данные контрагентов на сервер — вручную

Автодеплой возит только собранный интерфейс. Подготовленный набор переносится
отдельно и намеренно: он не входит в репозиторий, потому что репозиторий открыт,
а в данных сведения об учредителях, включая имена и личные идентификаторы.

```bash
# локально: собрать набор из выгрузки кейсодателя
python3 scripts/build_dataset.py          # → dataset/counterparties.json, ~3,3 МБ

# перенести на сервер
ssh <сервер> 'mkdir -p /opt/tt-hack/dataset'
scp dataset/counterparties.json <сервер>:/opt/tt-hack/dataset/

# проверить, что серверная часть его видит
curl -s <сервер>:8000/health | python3 -m json.tool   # ожидается count: 200
```

Путь переопределяется переменной `DATASET_PATH`. Без набора сервис не поднимается —
это сделано намеренно: работа с пустым набором выглядит как «у всех компаний ничего
нет» и неотличима от честного результата.

Набор пересобирается только при обновлении выгрузки кейсодателя. В остальное время
он на сервере неизменен и деплою не мешает.
