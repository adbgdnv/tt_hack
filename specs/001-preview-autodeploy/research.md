...
Запускается **после** rsync в релиз, но **до** свитча — проверяет новый релиз по временному
пути (`<base>/releases-staging/...`) ИЛИ (проще) после свитча, а при провале сразу
`preview_rollback.sh`. → выбираем: **до свитча через отдельный location**.

**Уточнение**: чтобы проверить релиз до того, как он стал текущим, добавить в Nginx временный
`location ^~ /__staging__/ { root /opt/tt-hack/releases-current-candidate; }` — усложняет.
**Проще и в духе Принципа III**: свитчим, затем smoke-check по боевому URL, при провале
`preview_rollback.sh` и exit 1. Окно «плохая версия текущая» — секунды, приемлемо для
превью-ревью (не прод). Зафиксировать в contracts как принятый компромисс.
