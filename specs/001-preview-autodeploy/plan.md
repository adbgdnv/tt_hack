...
`api`/`mcp` (FR-005). Секреты только в GitHub Secrets, не в логах (FR-008/014). Deploy-ключ —
минимальные права (FR-009). Одновременно пишет один запуск (FR-013).
...
deploy/
├── nginx/
│   └── tt-hack-review.conf   # существует — правка root на симлинк (или без правки, если путь тот же)
