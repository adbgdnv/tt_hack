...
- [X] T001 [P] Создать `deploy/PREVIEW-DEPLOY.md` — каркас ранбука с разделами: «Первичная
...
- [X] T002 [P] Создать `scripts/preview_common.sh` — пустой модуль с `set -euo pipefail`,
      shebang, комментарием назначения; будет источником общих функций (T003).

## Phase 2: Foundational (блокирует все user stories)

- [X] T003 Наполнить `scripts/preview_common.sh` общими функциями: `log()` (stdout, без
...
- [X] T004 [P] Создать `scripts/preview_smoke.sh` по `contracts/deploy-scripts.md`: аргументы
      ...
      печатать (FR-014).
- [X] T005 [P] Раздел «Первичная настройка сервера» в `deploy/PREVIEW-DEPLOY.md`: точные
...
- [X] T006 [P] Раздел «Секреты репозитория» в `deploy/PREVIEW-DEPLOY.md`: таблица
...
- [X] T007 [US1] Создать `scripts/preview_deploy.sh`: shebang + `source preview_common.sh`,
...
- [X] T008 [US1] В `scripts/preview_deploy.sh` — условная сборка фронта: если
      ...
      research R4).
- [X] T009 [US1] В `scripts/preview_deploy.sh` — сборка staging-каталога во временной
...
- [X] T010 [US1] В `scripts/preview_deploy.sh` — доставка: `RELEASE_ID=$(release_id)`, `rsync
      ...
      выходит.
- [X] T011 [US1] В `scripts/preview_deploy.sh` — финализация: `atomic_switch releases/$RELEASE_ID`
      ...
      в T015 — FR-007b.)
- [X] T012 [US1] Создать `.github/workflows/deploy.yml` по `contracts/deploy-workflow.md`:
...
- [X] T013 [US1] В `.github/workflows/deploy.yml` — шаги: `setup-node@v4` (условно по
...
- [X] T014 [US1] Раздел «Ручная выкладка» в `deploy/PREVIEW-DEPLOY.md`: `ssh ttdeploy@<host>`,
...
- [X] T015 [US2] В `scripts/preview_deploy.sh` сразу после `atomic_switch`, до `prune_releases`
...
- [X] T016 [US2] В `scripts/preview_deploy.sh` — fail-closed до switch: `npm`/`rsync`/сборка
...
- [X] T017 [US2] В `.github/workflows/deploy.yml` — убедиться, что при `conclusion != success`
...
- [X] T018 [US2] Коды smoke по каждому пути (`200 /` …) и строка отката печатаются в логе
      шага «Выложить превью» (их печатает `preview_smoke.sh` и `finalize()` через ssh);
      `$GITHUB_STEP_SUMMARY` даёт коммит, статус фронта, итог job и ссылку на этот лог.
      Секреты в summary и логи не попадают (проверяется в T025).
...
- [X] T019 [US3] Создать `scripts/preview_rollback.sh` по `contracts/deploy-scripts.md`:
...
- [X] T020 [US3] В `scripts/preview_rollback.sh` — после переключения `preview_smoke.sh`;
...
- [X] T021 [US3] Раздел «Откат» в `deploy/PREVIEW-DEPLOY.md`: примеры `preview_rollback.sh
...
- [X] T022 [P] Добавить в `.github/workflows/ci.yml` шаг проверки деплой-скриптов:
...
- [X] T023 [P] В `docs/architecture.md` (раздел «Ограничения, влияющие на архитектуру» /
...
- [X] T026 [P] Раздел «Диагностика» в `deploy/PREVIEW-DEPLOY.md`: частые сбои — протухший
