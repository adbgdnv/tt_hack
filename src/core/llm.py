...
def key_for(name: str) -> str:
    ...
    Два источника, и порядок важен. Именной (`OPENROUTER_API_KEY`) — свой
    у каждого провайдера, поэтому оба могут быть настроены одновременно, и это
    условие запасного пути. Общий `LLM_API_KEY` принадлежит тому провайдеру,
    который назван в `LLM_PROVIDER`: так было до появления второго, и старая
    настройка обязана продолжать работать. Отдавать общий ключ любому
    провайдеру нельзя — ключ Groq ушёл бы в OpenRouter.
    """
    load_env()
    провайдер = PROVIDERS.get(name)
    if провайдер is None:
        return ""
    именной = os.environ.get(провайдер.key_env, "")
    if именной:
        return именной
    if os.environ.get("LLM_PROVIDER", PRIMARY) == name:
