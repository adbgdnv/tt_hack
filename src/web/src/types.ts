...
export type CounterpartyReport = {
  ...
  /** Своя оценка по открытым данным. Необязательна по той же причине. */
  verdict?: OwnVerdict;
};

/**
 * Наша оценка контрагента — третья рядом с двумя чужими.
 *
 * `state` — четыре состояния, и четвёртое не уровень риска, а оговорка
 * к чистому: ничего не сработало, но не везде смотрели. Замерено на всех
 * 200 компаниях: таких 45 из 100 «чистых».
 */
export type OwnVerdict = {
  state: 'clean' | 'clarify' | 'attention' | 'unknown';
  wording: string;
  gaps: string[];
  checks_passed: number;
  checks_total: number;
