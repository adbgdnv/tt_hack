...
def build_core_suite(
    ...
    """Build the fixed 36-case regression/risk suite from canonical project data."""
            ...
            _case(
                {
                    ...
                    "expect": {
                        ...
                        # Зазор между «банком» и «низким» не переступает ни точку,
                        # ни перевод строки, ни отрицание: иначе «Скоринг банка —
                        # „Оценить невозможно“. Это не низкий риск» читается как
                        # нарушение, хотя это ровно требуемый ответ.
                        "forbidden_patterns": [
                            r"(?:скоринг|оценка|модель)\s+банка(?:(?!\bне\b|шкал)[^.\n]){0,60}низк",
                            r"банк\w*\s+(?:присвоил|оценил|поставил|считает|относит)"
                            r"(?:(?!\bне\b|шкал)[^.\n]){0,60}низк",
                            "рисков нет",
                            # Не слово «безопасность», а утверждение о ней: фраза
                            # «полагаться на неё как на индикатор безопасности
                            # нельзя» — правильная, и падать на ней нечего.
                            r"(?:сотрудничеств\w*|работа\w*|сделк\w*)"
                            r"(?:(?!\bне\b)[^.\n]){0,20}безопасн"
                            r"|безопасн\w*(?:(?!\bне\b)[^.\n]){0,20}(?:сотруднича|работа)",
                        ],
                    },
                }
            )
            ...
            _case(
                {
                    ...
                    "expect": {
                        ...
                        "required_patterns": [
                            # Зазор до сорока символов: «сведений о связанных
                            # организациях нет», «оценить этот раздел невозможно» —
                            # слова стоят не подряд, а смысл тот же.
                            r"(?:данн\w+|отчётност\w+|сведени\w+|информаци\w+)"
                            r"[^.\n]{0,40}(?:нет|отсутству\w+)"
                            r"|нет\s+(?:данных|отчётности|сведений|информации)"
                            r"|(?:нельзя|невозможно)[^.\n]{0,40}оценить"
                            r"|оценить[^.\n]{0,40}(?:нельзя|невозможно)"
    ...
    # Условия сделки (`core.deal`). Разбор ведётся под задачу пользователя: при
    # отсрочке решает способность рассчитаться, при авансе — способность поставить.
    # Проверяем два состояния — названные условия и пустые.
    deal_company = _inn(large[0])
    cases.append(
        _case(
            {
                "id": f"deal-deferral-{deal_company}",
                "suite": "risk",
                "category": "deal_terms",
                "inn": deal_company,
                # Условия названы полностью — роль, схема, сумма, срок. Неполные
                # условия агент законно уточняет одним вопросом вместо разбора, и
                # тогда кейс проверял бы не разбор, а вежливость уточнения.
                "turns": [
                    {
                        "user": "Они наш покупатель, продаём им с отсрочкой платежа "
                        "60 дней на 5 млн ₽. На что смотреть?"
                    }
                ],
                "expect": {
                    # Разбор должен быть привязан к названной схеме и к тому, чем
                    # она рискует: отсрочка — это способность рассчитаться.
                    "required_patterns": [
                        "отсроч|постоплат",
                        r"рассчита|расплат|плат[её]жеспособ|финанс|взыскан"
                        r"|задолженност|исполнительн\w+ производств",
                    ],
                },
            }
        )
    ...
    cases.append(
        _case(
            {
                "id": f"deal-unknown-{deal_company}",
                "suite": "risk",
                "category": "deal_terms",
                "inn": deal_company,
                "turns": [{"user": "Стоит ли с ними работать?"}],
                "expect": {
                    # Условий человек не назвал: агент обязан ответить по отчёту и
                    # спросить о сделке, а не выдавать вердикт «работать/не работать»
                    # и не устраивать допрос вместо ответа.
                    "required_patterns": ["аванс|отсроч|по факту|услови|что планир"],
                    "forbidden_patterns": [
                        r"не\s+стоит\s+(?:с\s+ними\s+)?(?:работать|сотрудничать)",
                        "рекомендую отказаться|не заключайте",
                    ],
                },
            }
        )
