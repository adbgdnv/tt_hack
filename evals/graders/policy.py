...
_NEGATION_BEFORE = re.compile(r"\b(?:не|ни)\s*(?:что\s*)?[«\"'„(\[]*\s*$", re.IGNORECASE)
...
_NEGATION_CLAUSE = re.compile(
    r"\b(?:не\s+(?:значит|означа\w*|говорит\w*|гарантир\w*|равн\w*|исключа\w*|синоним\w*)"
    r"|не\s+то\s+же(?:\s+самое)?"
