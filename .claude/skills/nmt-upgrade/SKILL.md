...
- `.claude/Next-Move-Theory-Canon/` — the canon
...
> **Локальное отличие этого репозитория.** Канон здесь лежит в
> `.claude/Next-Move-Theory-Canon/`, а не в корне, и пути в `nmt-*/SKILL.md`
> переписаны под этот префикс. Официальный установщик клонирует канон **в корень**
> и перезаписывает `nmt-*` скиллы — после каждого прогона `/nmt-upgrade` нужно
> перенести `Next-Move-Theory-Canon/` в `.claude/` и заново добавить префикс
> `.claude/` к путям канона в восьми `nmt-*/SKILL.md`.
...
1. **Locate the project root** — the folder that contains `.claude/Next-Move-Theory-Canon/`
